...
import com.dantefung.struts_baseaction.BaseAction2;
import com.opensymphony.xwork2.Action;

public class ImageCodeAction extends BaseAction2 implements Action {
	...
	public String execute() throws Exception {
		this.imgCode = genImageCode();
		return doDefault();
	}
	
	Random ran = new Random();
	
	public byte[] genImageCode(){
...
/*		try 
		{
			//5)把内存的图片输出给浏览器（作为正文内容）
			ServletOutputStream out = response.getOutputStream();
			//通知浏览器数据类型是什么  image/png
			response.setContentType("image/png");
			
			//参数一： 图片内容  参数二： 图片格式 png。jpeg  参数三： 输出的位置
			ImageIO.write(image, "png", out);
		} 
		catch (Exception e) 
		{
			request.setAttribute("msg", "系统正在维护中.....");
			e.printStackTrace(); // 在后台显示错误信息
		}*/
		
	}
	
	//随机生成颜色
	private Color genColor(){  //#  ff ff  ff  255 255 255
	...
	//生成四个随机中文
	private void genZhStr(Graphics g, Random ran) {
		...
		g.setColor(Color.BLUE);
		//字体
		g.setFont(new Font("黑体",Font.ITALIC,25));
		//画文字
